SELECT
 ...
 --     Look for the space in the column Games. It should give 5. Four characters for years + space = 5. Therefore -1 will remove the space to give us the correct year*/
 --       ,CHARINDEX(' ', Games)-1 AS 'Example 1'
 --     From the right side, REVERSE, account for all the characters until the space, to find the correct season.
 --       ,CHARINDEX(' ', REVERSE(Games))-1 AS 'Example 2'
        ,LEFT(Games, CHARINDEX(' ', Games) - 1) AS 'Year' -- Split column to isolate Year, based on space
--        ,RIGHT(Games,CHARINDEX(' ', REVERSE(Games))-1) AS 'Season' -- Split column to isolate Season, based on space. Removed since all = Summer
--        ,[Games]
